...
import com.ydys.elsbballs.api.TakeGoldInfoService;
...
public class TakeGoldInfoModelImp extends BaseModel implements TakeGoldInfoModel<TakeGoldInfoRet> {
    ...
    @Override
    public void takeTaskGold(TakeGoldInfo takeGoldInfo, IBaseRequestCallBack<TakeGoldInfoRet> iBaseRequestCallBack) {
        JSONObject params = new JSONObject();
        try {
            params.put("user_id", takeGoldInfo.getUserId());
            params.put("task_id", takeGoldInfo.getTaskId());
        ...
        mCompositeSubscription.add(takeGoldInfoService.takeTaskGold(requestBody)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
                .subscribe(new Subscriber<TakeGoldInfoRet>() {
                    @Override
