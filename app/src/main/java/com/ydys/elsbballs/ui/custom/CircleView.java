    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < 270; i++) {
            if (i % 2 == 0) {
                ...
                canvas.drawArc(oval, 134 + i, 1.35f, false, paint); // 绘制圆弧 1.35f是每个色块宽度
            }
        }
        ...
        for (int i = 0; i < currentDegree; i++) {
            if (!isGradual) {
                ...
                Integer color = (Integer) argbEvaluator.evaluate(i / 270f, Color.BLUE, Color.RED);//颜色插值器（level 11以上才可以用）
    ...
    Runnable runnable = new Runnable() {
        ...
        public void run() {
            ...
            if (currentDegree > 270)
                currentDegree = 0;
            invalidate();
            handler.postDelayed(runnable, 20);//6毫秒绘制一次（可以根据需要更改）
            currentDegree++;
        }
    };
