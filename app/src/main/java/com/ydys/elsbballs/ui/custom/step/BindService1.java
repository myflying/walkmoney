package com.ydys.elsbballs.ui.custom.step;
...
public class BindService1 extends Service implements SensorEventListener {
    ...
    public void onCreate() {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                nowBuSu = SPUtils.getInstance().getInt("now_step", (int) (Math.random() * 100));
                startStepDetector();
    ...
    private void addCountStepListener() {
        ...
        if (countSensor != null) {
            ...
            sensorManager.registerListener(BindService1.this, countSensor, SensorManager.SENSOR_DELAY_NORMAL);
            Log.i("计步传感器类型", "Sensor.TYPE_STEP_COUNTER");
        } else if (detectorSensor != null) {
            stepSensorType = Sensor.TYPE_STEP_DETECTOR;
            sensorManager.registerListener(BindService1.this, detectorSensor, SensorManager.SENSOR_DELAY_NORMAL);
        } else {
            addBasePedometerListener();
        }
    }
    ...
    private void updateNotification() {
        if (mCallback != null) {
            ...
            SPUtils.getInstance().put("now_step", nowBuSu);
        }
    }
